...
class All_in_One_SEO_Pack extends All_in_One_SEO_Pack_Module {
		   ...
		   "404_title_format"=> Array( 
			...
			$this->layout = Array(
				...
				'noindex' => Array(
						...
						'options' => Array( 'cpostnoindex', 'cpostnofollow', 'category_noindex', 'archive_date_noindex', 'archive_author_noindex', 'tags_noindex', 'search_noindex', 'tax_noindex', 'paginated_noindex', 'paginated_nofollow' )
					),
				'advanced' => Array(
	...
	function is_page_included() {
		...
		if( empty( $aioseop_options['aiosp_enablecpost'] ) ) {
			...
			if ( is_tax() ) {
				if ( !is_tax( $aioseop_options['aiosp_taxactive'] ) ) return false;
			} elseif ( is_category() ) {
			...
			} else if ( !is_singular( $wp_post_types ) && !is_front_page() && !is_post_type_archive( $wp_post_types ) ) return false;            
		}
	...
	function wp_head() {
			...
			if ( !empty( $opts ) ) {
				...
				if ( ( is_category() && !empty( $aioseop_options['aiosp_category_noindex'] ) ) || ( !is_category() && is_archive() && !is_tag() && !is_tax()
					...
					|| ( is_tax() && in_array( get_query_var( 'taxonomy' ), $aioseop_options['aiosp_tax_noindex'] ) ) )
