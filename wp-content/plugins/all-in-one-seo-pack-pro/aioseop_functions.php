...
if ( !function_exists( 'delete_term_meta' ) ) {
	if ( !isset( $wpdb->taxonomymeta ) )
		add_filter( 'delete_taxonomy_metadata', 'delete_tax_meta', 10, 3 );
	if ( !function_exists( 'delete_tax_meta' ) ) {
		function delete_tax_meta( $check, $term_id, $key ) {
...
if ( !function_exists( 'get_term_meta' ) ) {
	if ( !isset( $wpdb->taxonomymeta ) )
		add_filter( 'get_taxonomy_metadata', 'get_tax_meta', 10, 4 );
	if ( !function_exists( 'get_tax_meta' ) ) {
		function get_tax_meta( $check, $term_id, $key, $multi = false ) {
...
if ( !function_exists( 'update_term_meta' ) ) {
	if ( !isset( $wpdb->taxonomymeta ) )
		add_filter( 'update_taxonomy_metadata', 'update_tax_meta', 10, 5 );
	if ( !function_exists( 'update_tax_meta' ) ) {
		function update_tax_meta( $check, $object_id, $meta_key, $meta_value, $unique = false ) {
			$object_id = ( is_object( $object_id ) ) ? $object_id->term_id : $object_id;
			$m = get_option( 'tax_meta_' . $object_id );
